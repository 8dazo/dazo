    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for l in loops:
                ...
                pred = probs.argmax(-1)
                s = stats[l]
                s["correct"] += pred.eq(labels).sum().item()
                ...
                for i, meta in enumerate(metadata):
                    ...
                    d[0] += int(pred[i].item() == labels[i].item())
                    d[1] += 1

    elapsed = time.perf_counter() - started
    report = {"examples_per_second": sum(x["total"] for x in stats.values()) / max(len(loops), 1) / max(elapsed, 1e-9), "loops": {}}
    for l in loops:
